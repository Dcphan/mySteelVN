...
export async function loadFilterOptions(filter, date) {
  const url = new URL(`${hosting}/api/exporter-filtering-data`);
...
export async function fetchAndRenderTable(filter_fields, rows_fields, values_fields) {
  ...
  if (rows_fields.length > 1) {
    const url = new URL(`${hosting}/api/exporter-pivot-data`);
    ...
    const url = new URL(`${hosting}/api/exporter-single-pivot-summary`);
