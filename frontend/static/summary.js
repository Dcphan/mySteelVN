    ...
    async function loadCommodityOptions() {
      ...
      tomSelectInstance = new TomSelect("#commoditySelect", {
   ...
   function toggleCompanies(commodity) {
    ...
    async function fetchAndRenderTable() {
      ...
      commodities.forEach(c => url.searchParams.append("commodities", c));
      ...
      for (const [commodity, rows] of Object.entries(grouped)) {
        const total = rows.find(r => r.company === 'TOTAL');
        const others = rows.filter(r => r.company !== 'TOTAL');
        const commodityKey = makeSafeKey(commodity);
        ...
        tr.innerHTML = `
          <td class="px-4 py-2">
            <button id="toggle-${commodityKey}" onclick="toggleCompanies('${commodityKey}')" class="text-lg font-bold">+</button>
            ${commodity}
          </td>
        ...
        others.forEach(companyRow => {
          ...
          row.className = `child-${commodityKey} hidden`;
          row.innerHTML = `
